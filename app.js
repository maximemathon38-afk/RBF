...
async function syncUnitsToExpected(item){
  ...
  // Ajouter les exemplaires manquants jusqu'à la quantité prévue.
  const rows=[];
  ...
  // Supprimer les fiches au-delà de la quantité prévue.
  // Exemple : prévu 4 -> 3 => suppression de l'exemplaire n°4.
  const extraIds=existing.filter(u=>u.unit_no>expected).map(u=>u.id);
  if(extraIds.length){
    const {error}=await sb.from("equipment_units").delete().in("id",extraIds);
    if(error)throw error;
  }
...
async function saveItemAdd(){const c=currentContainer(),name=$("#itemName").value.trim(),expected=Math.max(0,+$("#itemExpected").value||0),present=Math.max(0,+$("#itemPresent").value||0),laser=!!$("#itemLaser").checked,serial=!!$("#itemSerial").checked||laser;if(!name){toast("Indique un nom.");return}const{data,error}=await sb.from("container_items").insert({container_id:c.id,catalog_id:null,name,expected,present,repair:0,requires_serial:serial,requires_laser_control:laser}).select().single();if(error){toast("Erreur : "+error.message);return}if(serial){const rows=[];for(let n=1;n<=expected;n++)rows.push({container_item_id:data.id,unit_no:n});if(rows.length){const r=await sb.from("equipment_units").insert(rows);if(r.error){toast("Matériel ajouté, mais erreur unités : "+r.error.message);return}}}await addHistory("item_add",c,name,expected,"Ajout propre à ce container");closeModal();await loadAll({quiet:true});toast("Matériel ajouté")}
